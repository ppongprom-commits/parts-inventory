...
export function AuthProvider({ children }) {
  ...
  async function loadMemberships() {
    ...
    let { data: allRows, error } = await supabase
      .from("shop_members")
      .select("member_id, shop_id, role, status, login_username, contact_name, expires_at, shops:shop_id (shop_name, subscription_status, subscription_plan)")
      ...
      if (pendingShopName) {
        ...
        if (!rpcError) {
          ...
          const refetch = await supabase
            .from("shop_members")
            .select(
              "member_id, shop_id, role, status, login_username, contact_name, expires_at, shops:shop_id (shop_name, subscription_status, subscription_plan)"
            )
            .eq("user_id", currentUser.id);
