...
export function AuthProvider({ children }) {
  ...
  async function loadMemberships() {
    ...
    let { data, error } = await supabase
      .from("shop_members")
      .select("member_id, shop_id, role, status, login_username, shops:shop_id (shop_name, subscription_status, subscription_plan)")
      .eq("status", "active");
    ...
    if (!error && (!data || data.length === 0)) {
      ...
      if (pendingShopName) {
        ...
        if (!rpcError) {
          ...
          // ดึงรายชื่ออู่ใหม่อีกครั้งหลังสร้างสำเร็จ
          const refetch = await supabase
            .from("shop_members")
            .select(
              "member_id, shop_id, role, status, login_username, shops:shop_id (shop_name, subscription_status, subscription_plan)"
            )
            .eq("status", "active");
          data = refetch.data;
          error = refetch.error;
