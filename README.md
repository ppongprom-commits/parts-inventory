...
### 31. คืนวันที่ 24 ก.ค. 2026 — ตอบคำถาม: join-as-support ยกเลิกได้มั๊ย + admin อื่นเห็นใครเข้าดู shop ไหนบ้าง

**คำถาม (จากภาพหน้า `/platform-admin` ของ "QA Test Shop (auto) - Worker 5"):** เข้าดูอู่อื่นผ่าน
join-as-support แล้วยกเลิกได้มั๊ย + platform_admin คนอื่นเห็นมั๊ยว่าใครเข้าดู shop ไหนอยู่

**ยกเลิกได้ (confirmed จากโค้ดจริง):** `platform_join_as_support(p_actor_user_id, p_shop_id)` RPC
insert แถวจริงเข้า `shop_members` (`role='manager'`, `status='active'`, `contact_name='Platform
Support'`) — ไม่ใช่ hidden session เพราะฉะนั้นยกเลิกด้วยวิธีเดียวกับลบสมาชิกทีมทั่วไป: เจ้าของ/ผู้จัดการ
อู่นั้นกด "🗑️ ลบ" ที่หน้า `/admin/team` → เรียก RPC `update_member_role` เปลี่ยน `status` เป็น
`'removed'` (soft-delete, ข้อมูลไม่หายจริง) **หมายเหตุ:** ไม่มี endpoint/ปุ่ม "leave support" เฉพาะ
ทางฝั่ง platform-admin เอง (`app/api/platform/**` ไม่มี route ไหนเป็น DELETE เลย) — การถอนตัวต้องผ่าน
หน้า `/admin/team` ของอู่นั้นเสมอ

**Admin อื่นเห็นได้เต็มๆ (confirmed):** ทุกครั้งที่ join-as-support จะเขียนเข้า `platform_audit_log`
ในธุรกรรมเดียวกับการ insert `shop_members` (ถ้าเขียน log ไม่สำเร็จ การ join ทั้งหมด rollback — ไม่มี
ทางเข้าโดยไม่มี log) แสดงผลผ่านแท็บ "📜 Activity Log" ในหน้า `/platform-admin` — ทั้ง 3 role เห็น
เหมือนกันหมด (`super_admin`/`support`/`analyst`, ตามมติเดิมของการ์ด "Platform admin role tiers": 19
ก.ค. 2026) filter ได้ทั้งตามคน (`admin_user_id`) และตามอู่ (`shop_id`) เป็น append-only แก้ไข/ลบไม่ได้
แม้แต่ super_admin เอง (`app/api/platform/audit-log/route.js` ไม่มี PATCH/DELETE โดยตั้งใจ)
นอกจากดูประวัติย้อนหลังแล้ว ยังเช็คได้ตรงๆว่าใคร "กำลัง" เป็นสมาชิกอยู่ตอนนี้จากหน้า `/admin/team` ของ
อู่นั้นเลย (จะเห็นชื่อ "Platform Support" อยู่ในรายชื่อทีมจริง)

**พบระหว่างตรวจ (gap บน production, ยังไม่ได้แก้):** ระบบ audit log ชุดนี้ทำงานได้ครบเฉพาะบน
staging (Supabase project `qmqabtrrubqcmafietsr`) — เช็ค production (`ktfnnmxrochfcjzifjlw`, branch
`main`) แล้วพบว่าตาราง `platform_audit_log` **ไม่มีอยู่จริง**, โค้ดที่ deploy จริงบน production ยังเป็น
เวอร์ชันเก่าที่ update `shop_members`/`shops` ตรงผ่าน service-role client (ข้าม audit trail ไปเลย),
`platform_admins` บน production ไม่มีคอลัมน์ `role` ด้วย และ `audit_log` (ของแต่ละร้าน) มี RLS policy
`"Allow public read audit_log"` (`USING (true)`) เปิดให้ผู้ใช้ที่ login แล้วอ่าน log ข้ามร้านได้หมด —
