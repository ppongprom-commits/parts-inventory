...
Repo นี้ทดสอบฟีเจอร์ login/RBAC ของ `parts-inventory` (github.com/ppongprom/parts-inventory, branch: staging) กับ staging deployment จริงบน Vercel Role ในระบบ: `owner`, `manager`, `supervisor`, `technician`, `assistant` (ระดับอู่) + `platform_admin` (ตารางแยก) — ดูรายละเอียดเต็มใน `../test_cases_login_rbac_parts_inventory.xlsx`
