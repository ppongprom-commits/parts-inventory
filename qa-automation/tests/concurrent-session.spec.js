...
async function pollSessionCount(filter, expectedCount, { timeoutMs = 5000, intervalMs = 300 } = {}) {
  ...
  while (Date.now() < deadline) {
    const { data } = await adminClient().from("user_sessions").select("session_id, user_id").eq(column, value);
    rows = data || [];
...
test("TC-302b user เดิม login เครื่องที่ 3 (เกิน maxDevicesPerUser=2) ไม่ถูกบล็อก แค่ evict session เก่าสุดแบบเงียบๆ (ไม่ signOut ใคร)", async ({
  ...
  try {
    ...
    const after2 = await pollSessionCount({ user_id: authUser.id }, 2);
    ...
    const after3 = await pollSessionCount({ user_id: authUser.id }, 2);
