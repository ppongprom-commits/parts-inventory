...
test("TC-302a คนที่ 4 (distinct user) login พร้อมกันในอู่ที่ tier=trial (cap=3) ต้องโดน force sign-out ทันที", async ({
  ...
  try {
    ...
    // แก้แล้ว (23 ก.ค. 2026, commit 53c6f92): /login อ่าน sessionError จาก useAuth() แล้ว
    // render เป็น .msg.error จริง — ยืนยันว่าผู้ใช้คนที่ 4 เห็นข้อความอธิบาย ไม่ใช่แค่เด้งเงียบๆ
    await expect(page4.locator(".msg.error")).toBeVisible({ timeout: 5000 });
  } finally {
