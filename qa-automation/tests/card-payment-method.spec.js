...
test("PAYMENT-001 ขายโดยไม่เลือกวิธีชำระเงิน -> error บังคับเลือก ไม่สร้าง part_sales", async ({ page }) => {
  ...
  // แก้ 22 ก.ค. 2026 (nightly automation): part_sales ไม่มีคอลัมน์ "id" — primary key จริงคือ
  // "sale_id" (ยืนยันจาก schema จริงผ่าน Supabase MCP) เดิม .select("id") ทำให้ query error และ
  // `data` เป็น null เสมอ ตรงกับอาการ "TypeError: Cannot read properties of null (reading 'length')"
  // ที่รายงานไว้ในการ์ด QA finding พอดี — เป็นบั๊กใน test เอง ไม่ใช่บั๊กแอป/RLS/schema ของ part_sales
