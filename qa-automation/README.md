# parts-inventory QA Automation — Login/RBAC + Regression + คืนวันที่ 21 ก.ค. 2026

Playwright test automation + agent runner สำหรับ [parts-inventory](https://github.com/ppongprom/parts-inventory) (branch: `staging`) รันกับ **staging deployment จริงบน Vercel + staging Supabase project จริง** (ไม่ mock เครือข่ายเหมือน `qa-tests/` ที่ root ของ repo ซึ่งสร้างไว้ให้รันในเซสชัน sandbox ที่ออกเน็ตไม่ได้)

เดิมไดเรกทอรีนี้ทำแค่ login/RBAC (TC-xxx) + job creation (JOB-xxx) — คืนวันที่ 21 ก.ค. 2026 เพิ่มชุดทดสอบฟีเจอร์ใหม่ 13 การ์ดคืนนั้นเข้ามาด้วย (TOS-xxx, JOBSTAT-xxx, AUDIT-xxx, MOVEPARTS-xxx, MOVEPART-xxx, PAYMENT-xxx, SALVAGE-xxx, FIELDSCAN-xxx, IMPORT-xxx, LABEL-xxx) — ดูหัวข้อ "คืนวันที่ 21 ก.ค. 2026" ท้ายไฟล์นี้สำหรับรายละเอียดชุดใหม่
...
├── fixtures/
│   ├── auth-helpers.js          # selector จริงจาก app/login, app/staff-login, RequireAuth, TosConsentGate
...
│   ├── job-helpers.js           # กรอกฟอร์ม /jobs/new + ปุ่ม state machine ที่ /jobs/[id] (คืน 21 ก.ค.)
│   ├── zone-helpers.js          # ขับ ZoneAutocomplete (custom search-select) — เพิ่มคืน 21 ก.ค.
...
│   ├── job-creation-photos.spec.js          # JOB-501-503 — path ของ TINY_PNG แก้แล้ว (ของเดิมชี้ผิดที่)
│   ├── job-creation-rbac.spec.js            # JOB-601-603
│   ├── job-creation-multitenancy.spec.js    # JOB-801
│   ├── card-tos-consent.spec.js             # TOS-001-004 (คืน 21 ก.ค.)
│   ├── card-job-status-tracking.spec.js     # JOBSTAT-001-008
│   ├── card-part-audit-history.spec.js      # AUDIT-001-005
│   ├── card-move-parts-unassigned.spec.js   # MOVEPARTS-001-002
│   ├── card-move-part-zone-action.spec.js   # MOVEPART-001-004
│   ├── card-payment-method.spec.js          # PAYMENT-001-003
│   ├── card-salvage-vehicle-intake.spec.js  # SALVAGE-001-006
│   ├── card-field-scanner-role.spec.js      # FIELDSCAN-001-006
│   ├── card-import-customers.spec.js        # IMPORT-001-004
│   └── card-zone-qr-and-print-labels.spec.js # LABEL-001-004 — ไม่ทดสอบสแกนกล้องจริง (ดูคอมเมนต์ในไฟล์)
├── scripts/
...
## คืนวันที่ 21 ก.ค. 2026 — ชุดทดสอบ real E2E สำหรับฟีเจอร์ใหม่คืนนั้น (13 การ์ด)

ไดเรกทอรี `qa-automation/` นี้เดิมมีอยู่แล้วบน branch `main` (จากงาน migration multi-tenant ก่อนหน้า)
แต่ไม่เคยถูก merge เข้า `staging` เลย — คืนนี้ port เข้ามาไว้บน `staging` แล้วขยายให้ครอบคลุมฟีเจอร์ใหม่
ทั้ง 13 การ์ดที่ทำในรอบ nightly QA คืนวันที่ 21 ก.ค. 2026 (ดูหัวข้อ "17. คืนวันที่ 21 ก.ค. 2026" ใน
README.md หลักของ repo สำหรับรายละเอียดฟีเจอร์แต่ละตัว) — **ทุก test ในหัวข้อนี้เขียนขึ้นโดยอ่าน
โค้ดจริงจาก selector/RLS policy/schema จริง (ยืนยันหลายจุดผ่าน Supabase MCP ตรงกับ staging project
จริง) แต่ยังไม่เคยถูกรันจริงสักครั้ง** เพราะ sandbox ที่เขียนไฟล์นี้ไม่มี network ออก `*.supabase.co`/
`*.vercel.app` (ข้อจำกัดเดียวกับที่อธิบายไว้บนสุดของไฟล์นี้) — **รอบแรกที่รันจริงบนเครื่อง/CI ควรเผื่อ
เวลาไล่ debug selector ที่อาจคลาดเคลื่อนไปบ้าง** ไม่ใช่ชุดที่ผ่านการ verify แบบ green run มาก่อนเหมือน
ชุด TC-xxx/JOB-xxx เดิมด้านบน

### ไฟล์ที่แก้เพื่อความเข้ากันได้ (ก่อน port เข้ามาไม่รองรับฟีเจอร์คืนนั้นเลย)

- `.env.example` — เพิ่ม `TEST_FIELDSCANNER_USERNAME/PIN` และ `TEST_FIELDSCANNER_EXPIRED_USERNAME/PIN`
- `fixtures/test-data.js` — เพิ่ม account `fieldScanner`/`fieldScannerExpired` + `pageAccess`/`allowedRoles` ใหม่
- `fixtures/auth-helpers.js` — เพิ่ม `expectExpiredAccountScreen`, `expectTosGateVisible/Hidden`,
  `expectTosNonOwnerMessage`, `acceptTosGate`
- `fixtures/job-helpers.js` — เพิ่ม helper ปุ่ม state machine (`clickStartStep`/`clickHoldStep`/...) และ
  `addWorkflowStepOnDetailPage` (คนละอันกับ `addWorkflowStep` เดิมที่กรอกตอนสร้างงานใหม่)
- `fixtures/db-client.js` — เพิ่ม `getUserIdByUsername`
- `fixtures/zone-helpers.js` (ใหม่) — ขับ `ZoneAutocomplete` component
- `scripts/setup-test-data.mjs` — เพิ่มบัญชี field_scanner (ปกติ + หมดอายุแล้วเสมอ) และ **seed
  `shop_tos_acceptances` ให้ owner ของทุก test shop ไว้ล่วงหน้า** (สำคัญมาก — ไม่งั้น
  `TosConsentGate` ที่เพิ่มเข้ามาคืนนั้นจะบล็อก suite เดิมทั้งหมดที่มีอยู่ก่อนแล้วโดยไม่ตั้งใจ)
- `scripts/teardown-test-data.mjs` — เพิ่ม field_scanner emails + ฟังก์ชัน `wipeShopChildRows()` ที่ลบ
  ลูกของแต่ละ shop (รวม 2 ตารางใหม่คืนนั้น: `shop_tos_acceptances`, `salvage_vehicles` ที่ไม่มี
  `ON DELETE CASCADE` เหมือน `parts`/`zones`/`customers` เดิม) ก่อนลบ shop เอง กัน FK violation ที่จะ
  แย่ลงกว่าเดิมเพราะตอนนี้ suite ใหม่เขียนข้อมูลลง shop หลักมากกว่าเดิมมาก
- `agent/priority-map.js` — เพิ่ม priority ของ ID ใหม่ทั้งหมด (TOS/JOBSTAT/AUDIT/MOVEPARTS/MOVEPART/
  PAYMENT/SALVAGE/FIELDSCAN/IMPORT/LABEL) และขยาย `ID_PATTERN` ให้จับ prefix ใหม่ด้วย

### บั๊กที่เจอในไฟล์เดิม (จาก `main`) ระหว่างพอร์ต — ไม่เกี่ยวกับฟีเจอร์คืนนั้นเลย

**`tests/job-creation-photos.spec.js` คำนวณ path ของ `TINY_PNG` ผิดตั้งแต่ต้น** — ใช้
`path.join(__dirname, "test-assets", "tiny.png")` จาก `tests/` แต่ไฟล์จริงอยู่ที่
`fixtures/test-assets/tiny.png` คนละที่กัน ทุก test ที่อัปโหลดรูปในไฟล์นี้ (JOB-501 ถึง JOB-503) จะ
โยน `ENOENT` ทันทีที่รันจริง — แก้เป็น `path.join(__dirname, "..", "fixtures", "test-assets", "tiny.png")`
แล้ว (ตรวจสอบว่าไฟล์จริงมีอยู่ที่ path ใหม่นี้แล้ว)

### Known gap ที่เจอระหว่างเขียน test ใหม่ (ไม่ใช่บั๊กที่ทดสอบทำให้เกิด — flag ไว้ให้ทีม dev ตัดสินใจ)

**`get_part_audit_history()` RPC ไม่รวม `field_scanner` ในรายชื่อ role ที่อนุญาต** (ดู
`db/audit_log_parts_coverage_migration.sql`) ทั้งที่ field_scanner แก้ไข part ได้จริง (`allowedRoles`
ของ `/edit/[id]` มี field_scanner) — แปลว่า field_scanner แก้ไขอะไหล่ได้ แต่เปิดดูประวัติการแก้ไขของ
ชิ้นที่ตัวเองเพิ่งแก้ไม่ได้เลย เหตุผลคือ RPC นี้เขียนขึ้นก่อนการ์ด Field Scanner Role จะเพิ่ม role ใหม่
เข้ามาในคืนเดียวกัน ไม่มีใครไปเพิ่ม field_scanner เข้า array ทีหลัง — `tests/card-part-audit-history.spec.js`
(AUDIT-005) ยืนยัน behavior ปัจจุบันไว้ (ไม่ใช่ assertion ว่าถูกต้อง)

### ตารางใหม่ที่ไม่มี `ON DELETE CASCADE` จาก `shops` (ต้องรู้ก่อนต่อยอด test เพิ่ม)

เหมือน `parts`/`zones`/`customers`/`jobs` เดิม — `shop_tos_acceptances` และ `salvage_vehicles` อ้าง
`shop_id references shops(shop_id)` แบบไม่มี cascade เขียน migration/test เพิ่มที่ insert ตารางพวกนี้
แล้วอยากให้ teardown ลบ shop ทดสอบได้สำเร็จ ต้องลบแถวลูกก่อนเสมอ (ดู `wipeShopChildRows()` ใน
`scripts/teardown-test-data.mjs` — เพิ่ม table name เข้า `CHILD_TABLES_IN_ORDER` ถ้าเพิ่มตารางใหม่อีก)
