...
Zone") — ปิด: ปุ่มสแกนเป็นแค่ทางเลือกเสริม พิมพ์ค้นหาเองได้ตามปกติ / เปิด: ซ่อนช่องค้นหาด้วยมือ
บังคับต้องสแกน QR เท่านั้นถึงจะบันทึกตำแหน่งได้ ใช้ผลเดียวกันทั้งที่ `/add`, `/edit/[id]`, และ
`/move-part/[id]` — **แก้ไข 24 ก.ค. 2026 (การ์ด "Platform-controlled shop features"):** เปิด/ปิด
ย้ายจาก owner/manager กดเองที่ `/admin` ไปเป็น **platform-admin เท่านั้น** (super_admin) จัดการจาก
`/platform-admin` แทน — หน้า `/admin` เหลือแค่แสดงสถานะปัจจุบัน (read-only) ผลพลอยได้: แก้บั๊กที่
`authenticated` ไม่เคยมี column-level UPDATE grant บน `shops.force_zone_scan_confirmation` มาตั้งแต่
ต้น (การกดปุ่มเดิมจาก browser จึง silently fail อยู่เสมอ) ไปในตัว เพราะตอนนี้เขียนผ่าน RPC
`platform_set_shop_feature()` (SECURITY DEFINER) แทนการ UPDATE ตรงจาก client — ดู
`db/platform_controlled_shop_features_migration.sql`
...
- Monetization gate — Pro tier ขึ้นไปเท่านั้น บังคับทั้งฝั่ง UI และฝั่ง RPC เอง
  (`set_accounting_module_enabled()` เช็ค `subscription_plan` ก่อนเสมอ ไม่ใช่แค่ UI ซ่อนปุ่ม)
- Informal report (`/admin/reports`) กับ formal ledger อ่านจากฐานข้อมูลชุดเดียวกัน (`part_sales`)
  ไม่ drift — verify ด้วย test ACC-012
- UI: `/admin/accounting` (ผังบัญชี + งวดบัญชี + journal entries, ปุ่มปิดงวด) ยังคง owner/manager
  operate เองเหมือนเดิมทุกประการ
- **แก้ไข 24 ก.ค. 2026 (การ์ด "Platform-controlled shop features"):** เปิด/ปิดโมดูลบัญชี
  (`shops.accounting_module_enabled`) ย้ายจาก owner/manager กดเองที่ `/admin`
  (`AccountingModuleSettingsCard` เดิม) ไปเป็น **platform-admin เท่านั้น** (super_admin — เหตุผล:
  entitlement ของร้าน เหมือน subscription_plan ไม่ใช่ preference ทั่วไป) จัดการจากแผง
  `/platform-admin` แทน — `set_accounting_module_enabled()` เพิ่มพารามิเตอร์ `p_actor_user_id`
  และเปลี่ยนเช็คสิทธิ์จาก `is_shop_member(owner/manager)` เป็น `platform_admins.role='super_admin'`
  แทน (side-effect logic เดิมทั้งหมด — seed ผังบัญชี + backfill journal entries ของงวดปัจจุบัน —
  ไม่เปลี่ยนแปลง) หน้า `/admin` เหลือแค่แสดงสถานะปัจจุบัน (read-only) + ลิงก์ไป `/admin/accounting`
  ถ้าเปิดอยู่ — RPC revoke จาก anon/authenticated แล้ว (เดิมเรียกตรงจาก browser ได้) เหลือแค่
  service_role เรียกผ่าน `app/api/platform/shops/[shopId]/accounting-module/route.js` — พบบั๊ก
  แทรกระหว่างทดสอบจริง: ฟังก์ชันลูก `fn_seed_default_chart_of_accounts`/
  `fn_backfill_current_period_sales` เช็คสิทธิ์ผ่าน `auth.uid()` ของตัวเองซ้ำอีกชั้น ซึ่ง
  เป็น NULL เสมอเมื่อเรียกผ่าน service_role (ทำให้เปิดโมดูลผ่าน API ใหม่ไม่ได้เลย 500) แก้ด้วย
  idiom เดียวกับ `db/platform_admin_rpc_auth_check_migration.sql` (เช็คเฉพาะเมื่อ auth.uid()
  ไม่ใช่ NULL) ดู `db/platform_controlled_shop_features_migration.sql`
- Test: `qa-automation/tests/accounting-module-core.spec.js` (12 tests, ผ่านทั้งหมดบน staging) +
  `qa-automation/tests/platform-controlled-shop-features.spec.js` (20 tests ใหม่ ครอบคลุม
  permission matrix ของทั้ง 3 feature ที่ย้ายมาเป็น platform-admin — ผ่านทั้งหมดบน staging)
...
- **Tier limit:** Starter/Founder/Trial = 1 สาขา (สร้างเพิ่มไม่ได้เลย), Pro = 2, Enterprise =
  ไม่จำกัด — enforce ทั้ง UI (แผง `/platform-admin`), API (`app/api/branches` POST,
  `lib/teamAuth.js` `checkBranchLimit`) และ DB (trigger `trg_branches_tier_limit`) — ตัวเลขซ้ำอยู่
  2 ที่ (`config/subscriptionTiers.js` `maxBranches` + SQL `fn_tier_max_branches`) ตาม pattern
  เดียวกับ Stock Value Cap Engine เดิม
- **Downgrade Enterprise→Pro ขณะมีสาขาเกิน limit:** ยอมให้ downgrade เสมอ ไม่บล็อก — platform admin
  เลือกเองผ่านแผง `/platform-admin` ว่าจะตั้งสาขาไหนเป็น `is_read_only` (สาขา default ตั้ง read-only
...
- **แก้ไข 24 ก.ค. 2026 (การ์ด "Platform-controlled shop features"):** จัดการสาขา (สร้าง/เปลี่ยนชื่อ/
  toggle read-only) ย้ายจาก shop owner/manager ทั้งหมด ไปเป็น **platform-admin เท่านั้น**
  (super_admin + support — งาน routine เหมือน join-as-support ไม่ใช่การตัดสินใจ billing) —
  `app/admin/branches/page.js` ถูก**ลบทิ้ง** การ์ด "🏬 จัดการสาขา" หายไปจาก `/admin` เจ้าของร้านเห็น
  ไม่เห็นเมนูนี้อีกต่อไป จัดการจากแผง `/platform-admin` (sub-panel "🏬 จัดการสาขา" ใน
  `ShopDetailPanel`) แทน — เพิ่มเกตใหม่ **`shops.branches_feature_enabled`** (boolean, default
  false) ที่ platform-admin (super_admin เท่านั้น) ต้องเปิดให้ร้านก่อน ถึงจะสร้างสาขาที่ 2 ขึ้นไปได้
  — **AND กับ tier limit เดิม ไม่ใช่แทนที่** (ต้องผ่านทั้ง 2 เงื่อนไข) enforce ทั้ง 3 ชั้นตามปกติ: API
  (`app/api/branches` POST เช็คก่อน `checkBranchLimit`), DB (`trg_check_branch_limit` เช็ค
  `branches_feature_enabled` ก่อนเช็ค tier limit) — grandfathering: ร้านที่มีมากกว่า 1 branch
  อยู่แล้วก่อน migration นี้ (ใช้งาน multi-branch จริงมาก่อน) ได้ `branches_feature_enabled=true`
  อัตโนมัติ ไม่ถูกล็อกฟีเจอร์ย้อนหลัง — `GET /api/branches` เดิม (branch switcher) **ไม่ถูกแตะเลย**
  ยังเป็นของ shop member ปกติ scope ตาม `shop_members` ของผู้เรียกเองเหมือนเดิม — endpoint ใหม่
  แยกต่างหากสำหรับ platform-admin ดูสาขาของร้านใดก็ได้: `GET /api/platform/shops/[shopId]/branches`
  (`super_admin`/`support`/`analyst` เห็นได้หมด — analyst read-only) ดู
  `db/platform_controlled_shop_features_migration.sql`
...
- **UI:** branch switcher ใน `components/AppShell.js` (mirror pattern เดียวกับ shop switcher เดิม
  — ซ่อนถ้าร้านมีสาขาเดียว, **ไม่ถูกแตะโดยการ์ด Platform-controlled shop features เลย** ยังเป็นของ
  shop member ทุก role เหมือนเดิม), sub-panel "🏬 จัดการสาขา" ใน `app/platform-admin/page.js`
  (สร้างสาขา/เปลี่ยนชื่อ/toggle read-only — แทนที่ `app/admin/branches/page.js` เดิมที่ลบไปแล้ว),
  assign role ต่อสาขาผ่าน `/admin/team` เดิม (extend ให้ส่ง `branch_id` ได้ ถ้าไม่ส่ง fallback ไปสาขา
  default ของร้านอัตโนมัติ — ร้านสาขาเดียวไม่ต้องแก้อะไรเลย)
...
**Test:** `qa-automation/tests/multi-branch-support.spec.js` (TC-MB-1..6, ยิง API ด้วย platform
admin token หลัง 24 ก.ค. 2026 แทน owner token เดิม) — data migration integrity, tier limits (API
layer), branch-scoped isolation, per-branch role, downgrade read-only, stock-cap whole-shop +
`qa-automation/tests/platform-controlled-shop-features.spec.js` (permission matrix เต็มรูปของ
super_admin/support/analyst รวม `branches_feature_enabled` AND-gate)
...
## 16. Platform Admin — Join-as-support & Activity Log — ✅ ใช้งานได้จริงบน staging, ⚠️ ยังไม่มีบน production (ตรวจสอบ 24 ก.ค. 2026)

**Join-as-support ("👤 เข้าดูอู่"):** platform admin (role `super_admin`/`support` เท่านั้น, `analyst`
ห้าม) กด join-as-support จากหน้า `/platform-admin` → RPC `platform_join_as_support` insert แถวจริง
เข้า `shop_members` (`role='manager'`, `status='active'`, `contact_name='Platform Support'`) —
**ไม่ใช่ hidden/stealth session** โผล่เป็นสมาชิกจริงในหน้า `/admin/team` ของอู่นั้นด้วย

- **ยกเลิกได้:** เจ้าของ/ผู้จัดการอู่นั้นกด "🗑️ ลบ" ที่หน้า `/admin/team` ตามปกติ (RPC
  `update_member_role` → `status='removed'`, soft-delete) — **ไม่มี** endpoint/ปุ่ม "leave support"
  แยกต่างหากทาง platform-admin เอง (`app/api/platform/**` ไม่มี route ไหนเป็น DELETE)
- **Admin คนอื่นเห็นได้:** ทุกครั้งที่ join-as-support เขียนเข้า `platform_audit_log` ในธุรกรรม
  เดียวกัน (เขียน log ไม่สำเร็จ = join ทั้งหมด rollback) แสดงผลผ่านแท็บ "📜 Activity Log" ในหน้า
  `/platform-admin` — ทั้ง 3 role เห็นเหมือนกัน, filter ตามคน/ตามอู่ได้, append-only (แม้ super_admin
  ก็แก้/ลบ log เองไม่ได้ — `app/api/platform/audit-log/route.js` ไม่มี PATCH/DELETE โดยตั้งใจ)

**⚠️ Gap บน production (ตรวจสอบผ่าน Supabase MCP โดยตรงกับ `ktfnnmxrochfcjzifjlw` แล้ว):** ระบบ
ข้างบนทำงานได้ครบเฉพาะ staging (`qmqabtrrubqcmafietsr`) เท่านั้น — production ยัง:
- ไม่มีตาราง `platform_audit_log` เลย (โค้ด deploy จริงบน `main` ยัง update `shop_members`/`shops`
  ตรงผ่าน service-role client ข้าม audit trail ไปเลย ไม่เคยเรียก RPC ที่เขียน log)
- `platform_admins` ไม่มีคอลัมน์ `role` (แปลว่ายังไม่มี role tier super_admin/support/analyst จริง)
- `audit_log` (ของแต่ละร้าน) ยังมี RLS policy เก่า `"Allow public read audit_log"` (`USING (true)`)
  — ผู้ใช้ที่ login แล้วอ่าน log ข้ามร้านได้หมด (ต้องแก้คู่กับการเพิ่มข้อมูลจริงเข้าไป ไม่งั้นยิ่งเพิ่ม
  ข้อมูลยิ่งเสี่ยง)

มีแผนพอร์ตให้ parity แล้ว (migration 2 ไฟล์ปรับให้เข้ากับ schema จริงของ production + โค้ด 10 ไฟล์
บน branch แยก `feature/production-audit-log-parity`) รอคุณอั้มยืนยันก่อนเริ่ม เพราะแตะ production
DB/โค้ดจริงโดยตรง
